...
# Function to specifically find "Case Report" in the manual tags
def find_case_reports(manual_tags):
    normalized_tags = [normalize(tag) for tag in manual_tags]
    if "casereport" in normalized_tags:
        return "Case Report"
    return ""
...
df['Publication Type'] = df.apply(
    lambda row: row['Publication Type'] if row['Publication Type'] == "Research Article" else find_case_reports(row['Manual Tags'].split(';')),
    axis=1
)

# Update the 'Publication Type' with other publication keywords if not already "Research Article" or "Case Report"
df['Publication Type'] = df.apply(
    lambda row: row['Publication Type'] if row['Publication Type'] in ["Research Article", "Case Report"] else find_other_pub_keywords(row['Manual Tags'].split(';')),
    axis=1
)
